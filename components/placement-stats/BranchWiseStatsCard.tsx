...
import type { Placement, BranchCompanyMapping } from "@/lib/placement-api";
...
const BranchWiseStatsCard: React.FC<BranchWiseStatsCardProps> = ({ branchCompany, placements }) => {
  ...
  const branchData = React.useMemo(() => {
    return branchCompany.map((b: BranchCompanyMapping) => ({
      branch: b.branch,
      selections: b.companies.reduce((sum: number, c: { company: string; count: number }) => sum + (c.count || 0), 0),
  ...
  const companiesForBranch = React.useMemo(() => {
    const branchObj = branchCompany.find((b: BranchCompanyMapping) => b.branch === activeBranch);
    if (!branchObj) return [];
  ...
  const branchCtcStats = React.useMemo(() => {
    ...
    const ctcs: number[] = [];
    placements.forEach((p) => {
      if (p.branch_counts.some((b: { branch: string; count: number }) => b.branch === activeBranch) && typeof p.ctc === "number" && !isNaN(p.ctc)) {
        ctcs.push(p.ctc);
      }
    });
