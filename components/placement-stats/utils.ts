...
export function groupSelectionsByMonth(placements: Placement[]): { month: string; selections: number }[] {
  ...
  placements.forEach((p: Placement) => {
    ...
    const total = Array.isArray(p.branch_counts)
      ? p.branch_counts.reduce((sum: number, b: { branch: string; count?: number }) => sum + (b.count || 0), 0)
      : 0;
...
export function getBranchPieDataFromMapping(branchCompany: BranchCompanyMapping[]) {
  // branchCompany: [{ branch, companies: [{ company, count }] }]
  return branchCompany.map((b) => ({
    branch: b.branch,
    count: b.companies.reduce((sum: number, c: { company: string; count: number }) => sum + (c.count || 0), 0),
